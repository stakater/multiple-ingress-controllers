...
sed "s|<%DOMAIN%>|${DOMAIN}|g" "${tmpl_path}/expose-controller-config-map.tmpl.yaml" > "${tmpl_path}/expose-controller-config-map.yaml"
tmpl_path="internal/exposecontroller"
sed "s|<%DOMAIN%>|${DOMAIN}|g" "${tmpl_path}/expose-controller-config-map.tmpl.yaml" > "${tmpl_path}/expose-controller-config-map.yaml"
echo "Substituting domain in externaldns-deployment.tmpl.yaml"
tmpl_path="externaldns"
sed "s|<%DOMAIN%>|${DOMAIN}|g" "${tmpl_path}/externaldns-deployment.tmpl.yaml" > "${tmpl_path}/externaldns-deployment.yaml"
